...
def load_data(train_folder, test_folder, view_list):
    num_view = len(view_list)

    data_tr_list = []
    for i in view_list:
        data_tr_list.append(np.loadtxt(os.path.join(train_folder, f"{i}_tr.csv"), delimiter=','))

    data_te_list = []
    for omic in ["mrna", "meth", "mirna"]:
        data_te_list.append(pd.read_csv(os.path.join(test_folder, f"{omic}.csv")).to_numpy())
    ...
    for data_mat in data_mat_list:
        data_tensor_list.append(torch.FloatTensor(data_mat))
    ...
    for data_tensor in data_tensor_list:
        data_train_list.append(data_tensor[idx_dict["tr"]].clone())
        data_all_list.append(torch.cat((data_tensor[idx_dict["tr"]].clone(),
                                        data_tensor[idx_dict["te"]].clone()), 0))
