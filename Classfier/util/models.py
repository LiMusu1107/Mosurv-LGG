...
from util.utils import load_model_dict
...
def load_model(num_view,model_path):

    if num_view == 3:
        dim_list = [50, 50, 50]
        dim_hvcdn = 8
        num_class = 2
        dim_he_list=[50, 50, 20]
        model_dict = init_model_dict(num_view, num_class, dim_list, dim_he_list, dim_hvcdn)
        model = load_model_dict(f"{model_path}/model_3omics", model_dict)
        ...
        dim_he_list=[50, 50, 20]
        model_dict = init_model_dict(num_view, num_class, dim_list, dim_he_list, dim_hvcdn)
        model = load_model_dict(f"{model_path}/model_mrna", model_dict)
